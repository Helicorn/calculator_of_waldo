...
JPA로 매핑된 포켓몬·회원 테이블은 코드 기준으로 관리합니다. LoL 매치·통계(`LOL_MATCH` 등)는 Swing `program` 등에서 채우며 JPA 매핑이 없습니다. **LoL 자유게시판**(`LOL_BOARD`, `LOL_BOARD_COMMENT`)은 `T_USER`·게시글과 FK로 연결되며, JPA 엔티티는 추후 추가 예정입니다.
...
인덱스: `IDX_LOL_BOARD_01 (DEL_YN, CREATED_AT DESC)`, `IDX_LOL_BOARD_02 (USER_NO)`
