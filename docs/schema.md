...
JPA로 매핑된 포켓몬·회원 테이블은 코드 기준으로 관리합니다. LoL 매치·통계(`LOL_MATCH` 등)는 Swing `program` 등에서 채우며 JPA 매핑이 없습니다. **LoL 자유게시판**은 `LOL_BOARD_HEAD`(말머리)·`LOL_BOARD`·`LOL_BOARD_COMMENT`와 `T_USER` FK로 연결되며, JPA 엔티티는 `com.waldo.games.lol.board`에 있습니다.
