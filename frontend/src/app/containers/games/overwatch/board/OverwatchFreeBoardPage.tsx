...
import { OVERWATCH_BOARD_API } from "@/app/containers/games/overwatch/board/overwatchBoardApi";
...
/** Overwatch 자유게시판 — GamePageLayout `?detail=free-board` 본문 */
export function OverwatchFreeBoardPage() {
  ...
  const loadPosts = useCallback(
    async (pageIndex: number, signal: AbortSignal) => {
      ...
      setError(null);
      try {
        const url = `${OVERWATCH_BOARD_API}?page=${pageIndex}&size=${BOARD_LIST_PAGE_SIZE}&sort=createdAt,desc`;
    ...
    if (sessionUser === undefined) {
      void apiGetJsonIfOk<LoginResponse>("/api/me").then((me) => {
        ...
        if (me === null) {
          ...
          router.push(overwatchBoardWriteHref());
        }
      });
      return;
    }
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    try {
      const results = await Promise.all(
        ids.map((boardId) =>
          fetch(`${OVERWATCH_BOARD_API}/${boardId}`, {
            method: "DELETE",
            credentials: "include",
          }),
        ),
          ...
          <ul className={lolBoardListDividerUlClass}>
            {posts.map((post) => (
              ...
              >
                ...
                <Link
                  href={overwatchBoardDetailHref(post.boardId)}
